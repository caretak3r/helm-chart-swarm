#!/usr/bin/env bash
# cert-manager self-signed Issuer -> CA Cert -> CA Issuer -> leaf Cert smoke assertion.
# Verifies: CA Certificate Ready, leaf Certificate Ready, TLS Secret keys,
#           HTTPS serving with --cacert, SAN matches host.
# All Certificate/Issuer resources are created by cluster.preinstall raw_manifests.
...
CA_CERT_NAME="chart-test-swarm-ca"
LEAF_CERT_NAME="sample-tls"
DOMAIN="${RELEASE}.${NS}.svc"

kctl() { kubectl ${KUBE_CONTEXT:+--context "$KUBE_CONTEXT"} "$@"; }

echo "==> Waiting for CA Certificate '${CA_CERT_NAME}' Ready (3m max)"
kctl -n "${NS}" wait certificate "${CA_CERT_NAME}" --for=condition=Ready --timeout=3m
echo "PASS: CA Certificate Ready=True"

echo "==> Waiting for leaf Certificate '${LEAF_CERT_NAME}' Ready (3m max)"
kctl -n "${NS}" wait certificate "${LEAF_CERT_NAME}" --for=condition=Ready --timeout=3m
echo "PASS: leaf Certificate Ready=True"
...
# Extract ca.crt from the CA Secret (the root CA that signed the intermediate)
...
echo "PASS: cert-manager self-signed CA chain integration verified"
